...
static void ggml_cuda_mul_mat(ggml_backend_cuda_context & ctx, const ggml_tensor * src0, const ggml_tensor * src1, ggml_tensor * dst) {
    ...
    if (split) {
        ...
        for (int id = 0; id < ggml_backend_cuda_get_device_count(); ++id) {
            ...
            use_mul_mat_q           = use_mul_mat_q             && ggml_cuda_should_use_mmq(src0->type, cc, warp_size, ggml_cuda_info().devices[id].smpbo, src1->ne[1], /*n_experts=*/0);
        ...
        use_mul_mat_q           = use_mul_mat_q             && ggml_cuda_should_use_mmq(src0->type, cc, warp_size, ggml_cuda_info().devices[ctx.device].smpbo, src1->ne[1], /*n_experts=*/0);
        use_mul_mat_f           = use_mul_mat_f             && ggml_cuda_should_use_mmf(src0->type, cc, warp_size, src0->ne, src0->nb, src1->ne[1], /*mul_mat_id=*/false);
        use_mul_mat_vec_f       = use_mul_mat_vec_f         && ggml_cuda_should_use_mmvf(src0->type, cc, src0->ne, src0->nb, src1->ne[1]);
        any_gpus_with_slow_fp16 = any_gpus_with_slow_fp16   || !fast_fp16_hardware_available(cc);
    }
...
static void ggml_cuda_mul_mat_id(ggml_backend_cuda_context & ctx, ggml_tensor * dst) {
    ...
    if (src1->type == GGML_TYPE_F32 && dst->type == GGML_TYPE_F32) {
        ...
        if (ggml_cuda_should_use_mmq(src0->type, cc, warp_size, ggml_cuda_info().devices[ctx.device].smpbo, ne12, /*n_experts=*/ne02)) {
        ...
        case GGML_OP_SET_ROWS:
            {
                return (op->type == GGML_TYPE_F32 || op->type == GGML_TYPE_F16 || op->type == GGML_TYPE_BF16 ||
                       ...
                       op->type == GGML_TYPE_TURBO2_0 || op->type == GGML_TYPE_TURBO2H_0 ||
                       op->type == GGML_TYPE_TURBO3_0 || op->type == GGML_TYPE_TURBO3H_0 ||
                       op->type == GGML_TYPE_TURBO4_0 || op->type == GGML_TYPE_TURBOP3_0 ||
                       op->type == GGML_TYPE_TURBOP4_0 || op->type == GGML_TYPE_TURBOP5_0) &&
